...
namespace MaterialDesignIconsEnum
{
    class Program
    {
        ...
        {
            Console.WriteLine("MaterialDesignIconsEnum - Version 1.0.0 Copyright (C) Simon Mourier 2016-" + DateTime.Now.Year + ". All rights reserved.");
            Console.WriteLine("");

            OptionHelp = CommandLineUtilities.GetArgument("?", false);
            OptionInputPath = CommandLineUtilities.GetArgument<string>(0, null);
            OptionOutputPath = CommandLineUtilities.GetArgument(1, "MaterialDesignIconsEnum.cs");

            if (OptionHelp || Environment.GetCommandLineArgs().Length == 1)
            {
                Console.WriteLine("Format is MaterialDesignIconsEnum.exe <materialdesignicons.less input file path>");
                Console.WriteLine("");
                return;
            }
            ...
            using (var writer = new StreamWriter(OptionOutputPath, false))
            {
                ...
                writer.WriteLine("\t/// Material Design Icons Resources.");
                writer.WriteLine("\t/// </summary>");
                writer.WriteLine("\tpublic static partial class MaterialDesignIconsResource");
                writer.WriteLine("\t{");
                for (int i = 0; i < enums.Count; i++)
                {
                    var kv = enums[i];
